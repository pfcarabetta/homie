...
Write ONLY the summary — no questions, no conversational language, no greetings, no markdown. Start with the specific appliance/system and model if known. End with the likely cause or symptom. This text goes straight to service providers so they can decide whether to bid.`,
  ...
  // Correlation text — USER-ONLY content. AI follow-up questions
  // routinely reference adjacent systems ("any backup in the sink?",
  // "check the drain line", "is the water heater making noise?") that
  // would widen the card into unrelated items as the chat progresses.
  // Only the homeowner's own words count. Mirrors the BusinessChat
  // chatCorrelationText pattern. Also EXCLUDES the AI diagnosis
  // summary — that text is generated by Claude AFTER the chat ends and
  // often names related appliances as context, which would add items
