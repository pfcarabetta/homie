      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        {views.map(v => (
          <button key={v.id} onClick={() => setView(v.id)} style={{
            ...
            border: view === v.id ? `2px solid ${O}` : '1px solid var(--bp-border)',
            background: view === v.id ? `${O}08` : 'var(--bp-card)',
      ...
      {view === 'monthly' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {report.by_month.length === 0 ? (
            ...
            return (
              <div key={m.month} style={{ background: 'var(--bp-card)', borderRadius: 10, border: '1px solid var(--bp-border)', padding: '14px 18px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
