        ...
        <div style={{ display: 'grid', gap: 8, marginBottom: 20 }}>
          {ROLE_OPTIONS.map(r => (
            <label key={r.value} style={{
              ...
              border: role === r.value ? `2px solid ${O}` : '2px solid var(--bp-border)',
              background: role === r.value ? `${O}08` : 'var(--bp-card)',
            }}>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        {members.map(m => {
          ...
          return (
            <div key={m.id} style={{
              background: 'var(--bp-card)', borderRadius: 10, border: '1px solid var(--bp-border)', padding: '16px 20px',
                ...
                {editingId === m.id ? (
                  <>
                    <select value={editRole} onChange={e => setEditRole(e.target.value)}
                      style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--bp-border)', fontSize: 13 }}>
                    ...
                    <button onClick={() => setEditingId(null)}
                      style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid var(--bp-border)', background: 'var(--bp-card)', fontSize: 13, cursor: 'pointer', color: D }}>Cancel</button>
                  </>
                ) : (
                  <>
