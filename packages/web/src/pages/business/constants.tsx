...
export function getPlanTiersOrdered(pricing: PricingConfig) {
  ...
  return [
    ...
    { plan: 'professional', limit: bp.professional?.maxProperties  ?? 150,  label: 'Professional', price: `$${bp.professional?.base ?? 99}/mo + $${bp.professional?.perProperty ?? 10}/property` },
    { plan: 'business',     limit: bp.business?.maxProperties      ?? 500,  label: 'Business',     price: `$${bp.business?.base ?? 249}/mo + $${bp.business?.perProperty ?? 10}/property` },
...
export function getBillingPlans(pricing: PricingConfig) {
  const bp = pricing.business;
  return [
    { plan: 'starter',      label: 'Starter',       price: bp.starter?.base      ?? 0,   promoPrice: bp.starter?.promoBase      ?? null, promoLabel: bp.starter?.promoLabel      ?? null, perProperty: bp.starter?.perProperty      ?? 10, maxProperties: bp.starter?.maxProperties      ?? 10,  maxMembers: bp.starter?.maxTeamMembers      ?? 1,  features: [`Up to ${bp.starter?.maxProperties ?? 10} properties`, '1 user', 'Unlimited AI diagnostics', 'Automated provider outreach', 'Preferred vendors (up to 5)', 'iCal calendar sync', 'Basic cost tracking'] },
    { plan: 'professional', label: 'Professional',   price: bp.professional?.base ?? 99,  promoPrice: bp.professional?.promoBase ?? null, promoLabel: bp.professional?.promoLabel ?? null, perProperty: bp.professional?.perProperty ?? 10, maxProperties: bp.professional?.maxProperties ?? 150, maxMembers: bp.professional?.maxTeamMembers ?? 5,  features: [`Up to ${bp.professional?.maxProperties ?? 150} properties`, `${bp.professional?.maxTeamMembers ?? 5} team members`, 'Priority dispatch (phone, text, email)', 'Track PMS sync', 'Vendor scorecards & cost reporting', 'Auto-dispatch rules & Slack alerts', 'White-label guest portal (QR codes & property links)', 'Branded estimate PDFs'] },
    { plan: 'business',     label: 'Business',       price: bp.business?.base     ?? 249, promoPrice: bp.business?.promoBase     ?? null, promoLabel: bp.business?.promoLabel     ?? null, perProperty: bp.business?.perProperty     ?? 10, maxProperties: bp.business?.maxProperties     ?? 500, maxMembers: bp.business?.maxTeamMembers     ?? 9999, features: [`Up to ${bp.business?.maxProperties ?? 500} properties`, 'Unlimited team members with roles', 'Dedicated outreach manager', 'Priority email support with SLA', 'Custom workspace onboarding', 'Quarterly business review'] },
  ];
}
