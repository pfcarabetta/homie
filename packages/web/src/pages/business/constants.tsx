...
export function getBillingPlans(pricing: PricingConfig) {
  const bp = pricing.business;
  return [
    { plan: 'starter',      label: 'Starter',       price: bp.starter?.base      ?? 0,   promoPrice: bp.starter?.promoBase      ?? null, promoLabel: bp.starter?.promoLabel      ?? null, perProperty: bp.starter?.perProperty      ?? 10, maxProperties: bp.starter?.maxProperties      ?? 10,  maxMembers: bp.starter?.maxTeamMembers      ?? 1,  features: [`Up to ${bp.starter?.maxProperties ?? 10} properties`, '1 user', 'Unlimited AI diagnostics', 'Automated provider outreach', 'Preferred vendors (up to 5)', 'Basic cost tracking'] },
    { plan: 'professional', label: 'Professional',   price: bp.professional?.base ?? 99,  promoPrice: bp.professional?.promoBase ?? null, promoLabel: bp.professional?.promoLabel ?? null, perProperty: bp.professional?.perProperty ?? 10, maxProperties: bp.professional?.maxProperties ?? 50,  maxMembers: bp.professional?.maxTeamMembers ?? 5,  features: [`Up to ${bp.professional?.maxProperties ?? 50} properties`, `${bp.professional?.maxTeamMembers ?? 5} team members`, 'PMS sync (Track + iCal)', 'Vendor scorecards & cost reporting', 'Auto-dispatch & Slack alerts', 'Guest issue portal with QR codes', 'Branded estimate PDFs'] },
    { plan: 'business',     label: 'Business',       price: bp.business?.base     ?? 249, promoPrice: bp.business?.promoBase     ?? null, promoLabel: bp.business?.promoLabel     ?? null, perProperty: bp.business?.perProperty     ?? 10, maxProperties: bp.business?.maxProperties     ?? 150, maxMembers: bp.business?.maxTeamMembers     ?? 15, features: [`Up to ${bp.business?.maxProperties ?? 150} properties`, `${bp.business?.maxTeamMembers ?? 15} team members with roles`, 'Role-based access controls', 'Priority email support', 'Custom workspace onboarding', 'Quarterly business review'] },
  ];
}
